...
In this section, we will use the test data set to fit all candidate models we got from last section. We will calculate the test Mean Squared Error (MSE), $$R^2$$ for each model. The model with the smallest MSE will be the best fit model.

## Exam model fit on First Linear Regression Model

```{r fit_first_regression}
pred1<- predict(fit_lasso, newdata=test)
postResample(pred1, obs = test$shares)
```
...
postResample(pred2, obs = test$shares)
```
...
postResample(pred3, obs = test$shares)
```
...
postResample(pred4, obs = test$shares)
```
